  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', color: '#333' }}>
      <div style={{ backgroundImage: `url(${heroImg})`, backgroundSize: 'cover', backgroundPosition: 'center', padding: '100px 20px', textAlign: 'center', color: '#fff' }}>
        ...
        <button onClick={handleStart} style={{ padding: '15px 30px', fontSize: '1em', backgroundColor: '#1E3A8A', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer' }}>Register Now</button>
      </div>

      {/* Instructor Stats */}
      <div style={{ textAlign: 'center', marginTop: 40 }}>
        <h2 style={{ color: '#1E3A8A' }}>Join 10,000+ instructors worldwide</h2>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', marginTop: 40, padding: '0 20px' }}>
        {benefits.map((b, i) => (
          <div key={i} style={{ width: 280, margin: 15, padding: 20, textAlign: 'center', borderRadius: 8, backgroundColor: '#fafafa', animation: 'fadeIn 0.8s ease forwards', animationDelay: `${i * 0.2}s`, opacity: 0 }}>
            ...
            <h3 style={{ fontSize: '1.3em', marginBottom: 10, color: '#1E3A8A' }}>{b.title}</h3>
      ...
      {showForm && (
        <div style={{ maxWidth: 600, margin: '60px auto', padding: 20, backgroundColor: '#fff', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.1)' }}>
          <h2 style={{ color: '#1E3A8A', textAlign: 'center' }}>Apply to Teach</h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" style={{ width: '100%', padding: 12, backgroundColor: '#1E3A8A', color: '#fff', border: 'none', borderRadius: 6, marginTop: 10 }}>Submit Application</button>
          </form>
        </div>
      )}
      ...
      <div style={{ maxWidth: 800, margin: '60px auto', textAlign: 'center' }}>
        <h2 style={{ color: '#1E3A8A', marginBottom: 20 }}>Watch how easy it is to get started</h2>
        <iframe
          width="100%"
          height="400"
      ...
      <div style={{ maxWidth: 800, margin: '60px auto', padding: '0 20px' }}>
        <h2 style={{ color: '#1E3A8A', textAlign: 'center', marginBottom: 30 }}>Frequently Asked Questions</h2>
        {faqs.map((faq, i) => (
      ...
      <div style={{ maxWidth: 1000, margin: '60px auto', padding: '0 20px' }}>
        <h2 style={{ color: '#1E3A8A', textAlign: 'center', marginBottom: 30 }}>What Our Instructors Say</h2>
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', gap: 30 }}>
          {testimonials.map((t, i) => (
            <div key={i} style={{ flex: '1 1 0', maxWidth: 300, padding: 20, background: '#f9f9f9', borderRadius: 8, textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              ...
              <h4 style={{ marginTop: 10, fontSize: 16, color: '#1E3A8A' }}>{t.name}</h4>
            </div>
          ))}
        </div>
      </div>
